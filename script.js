...
(() => {
  ...
  async function loadCentralRules() {
    const response = await fetch(new URL("./data/rules.json", import.meta.url), { cache: "no-store" });
    if (!response.ok) {
      throw new Error(`HTTP ${response.status} beim Laden des Regelwerks.`);
    }
  ...
  function evaluateWithCentralRules(change) {
    if (!centralRules) {
      throw new Error("Das zentrale Regelwerk ist nicht geladen.");
    }
    return evaluateChange(change, centralRules);
  }
